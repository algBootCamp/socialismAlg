...
public class Program8 {
    public static void main(String[] args) {
        ...
        System.out.println(program8.threeSum(nums));
    }
    ...
    private List<List<Integer>> threeSum(int[] nums) {
        ...
        if(nums != null && nums.length >= 3) {
            ...
            for(int i = 0; i < nums.length; ++i) {
                int target = -nums[i];
                /*
                 * why start with i+1 instead of 0:
                 * because if we can find a index j in range [0 ~ i-1] satisfy given condition,
                 * we must already add it when iterate j
                 */
